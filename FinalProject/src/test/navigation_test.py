...
def sideUSensor(distanceToWall):
    sample_int = 0.2 #sample every 200 ms
    wall_dist = distanceToWall #20 cm from wall
...
def begin_threading_instances():
    ...
    #fix this last
    run_in_backgroud(lambda: sideUSensor(), lambda: frontUSensor()) # Both functions that we want running at the same time
